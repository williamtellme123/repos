...
-- The select clause expressions : case (shows us what where is doing)
    select case 
              when lastname = 'MORALES' 
                  then 'T' else 'F' 
              end as SameAsWhereTest
          , firstname
          , lastname 
    from customers 
    where lastname = 'MORALES';
    
select max(length(description)) from park;


select park_name, regexp_substr(park_name,'Park')
from park
where regexp_like(park_name,'Park');
...
--        pattern1    regular expression. Required.
      select regexp_instr('A B C b D b E B','B',1,2,0,'i') as address from dual;
...
-- c. Find location of either string '10' or string 'S' at different starting position
      select regexp_instr('S 10 | Main Street Drain No. 101','10|\|',1,2) as address from dual;
      ...
      select regexp_instr('S 10 Main Street Drain No. 101','10|S',2,1) as address1, 
             ...
             substr('S 10 Main Street Drain No. 101',2,2) as address3
      from dual;
-- -----------------------------------------------------------------------------
-- d. Find location of any of several characters
      select regexp_instr('S 10 Main Street Drain No. 101','N|S|M',1,1) as address1,
              ...
              regexp_substr('S 10 Main Street Drain No. 101','[SMN]',1,1) as address3
      from dual;
...
-- g. What about the spaces
      select regexp_instr('S 10 Main Street Drain No. 101','[1 oN.]+',20,1) as a0,
      regexp_substr('S 10 Main Street Drain No. 101','[1 oN.]+',20,1) as a1 
      from dual;
      
      select regexp_substr('an apple a day means no pies','(apple)( )(a)') a,
      regexp_instr('an apple a day means no pies','apple') b
      from dual;
...
-- i. What if we wanted to find a longer string but forgot the brackets
      select regexp_instr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                         ,'[apple]+',1,1) as address from dual;
-- -----------------------------------------------------------------------------
--  j. OK we fixed that by adding brackets. Now lets add {quantity} : Line 11 
      ...
      select regexp_instr('123 Mapleu Avenue, Austin, TX 78729 (512) 311-4545','[eu, ]{1,3}',1,1) as address1, 
              regexp_substr('123 Mapleu Avenue, Austin, TX 78729 (512) 311-4545','[eu, ]{1,3}',1,1) as address2
      from dual;      
-- ------------------------------------------------------------------------------------------------------------
...
--        p1              StartPosition Optional. Defaults to 1.
      select    regexp_substr('123 Maple Apple','[apple]{4}',1,2,'i') as address1,
      regexp_instr('123 Maple Apple','[apple]{4}',1,2) as address1
      from dual;
-- -----------------------------------------------------------------------------
--    STUDENT EXERCISE
...
-- b. Find same pattern using : matching set, or range, or Character Class 
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[0123456789]{3}',1,2) as address1 
      from dual;
--  Change [0123456789] to [0-9]              : Table 17-3
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
       ,'[0-9]{3}',1,2) as address1 
      from dual;
--  Change [0123456789] to [:DIGIT:]          : Table 17-2
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[[:alpha:]]{3}',1,2) as address1 
      from dual;
-- ------------------------------------------------------------------------------------------------------------
-- c. Find using : quantities        : Line 10     {this means contiguous trues)
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[ a-zA-Z,0-9]+',1,1) as address1 
      from dual;
--    Find using : quantities        : Line 11
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[a-zA-Z]{3}',1,1) as address1 
      from dual;
...
-- e. Find pattern
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[0-9()]',1,7) as a1,
              regexp_instr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[0-9()]',1,7) as a1 
      from dual;
-- ------------------------------------------------------------------------------------------------------------
-- f. Find pattern
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[0-9() -]{4,}',1,2) as address1
      from dual;
-- ------------------------------------------------------------------------------------------------------------
-- g. Find pattern
      select regexp_substr('123 Maple Avenue, Austin, TX 78729 (512) 311-4545'
                          ,'[0-9( )-]{7,}',1,1) as address1
      from dual;
-- ------------------------------------------------------------------------------------------------------------
...
select regexp_substr(description,'([[:alpha:]]+).',1,1,'n')
from park
where regexp_like(description,'.','n');

-- ------------------------------------------------------------------------------------------------------------
-- 3. REGEXP_REPLACE (s1,pattern1,rep1,p1,o1,m1 
-- ------------------------------------------------------------------------------------------------------------ 
-- a. replace the word "light" with "sound"
    select regexp_replace('But, soft! What light through' 
...
--  Simple phone examples   line : 14
  ...
  select 
            regexp_substr(description,'[[:digit:]]{2,3}'),
            regexp_substr(description,'[[:digit:]]{3}')
  from park
  where regexp_like(description,'[[:digit:]]{2,3}')
    or regexp_like(description,'[[:digit:]]{3}');
...
-- 1. ---------------------------------------------------------------------    
    ...
    select regexp_substr('+1 38 04143 235 98','\+[0-9 ]+')    
    from dual;
-- 2. ---------------------------------------------------------------------    
...
-- 3. ---------------------------------------------------------------------    
    select 
      park_name, regexp_substr(description,'[0-9]{3}-[0-9]{4}|[0-9]{3}\.[0-9]{4}|[0-9]{3} [0-9]{4}')
    from park
    where regexp_like(description,'[0-9]{3}-[0-9]{4}|[0-9]{3}\.[0-9]{4}|[0-9]{3} [0-9]{4}');
    
    insert into park values('My Park', null,null, '(800) 333 4444') ;
...
-- a. What does this return
      select regexp_substr('shrororororosasasasae sells sea shells down by the seashore',
            ...
            regexp_substr('she sells sea shells down by the seashore',
                      's[eashor]+e' ) the_result1,
...
-- START HERE ON TUESDAY AND HAVE A GREAT WEEKEND AND.   
-- ------------------------------------------------------------------------------------------------------------
...
select zip, regexp_substr(trim(zip) , '[0-9]{5}')
from tuition_stg
where regexp_like (trim(zip), '[0-9]{5}');

--  Zip Type 1: anything that starts with at least 5 digits
--   digit
--   space
--   dash
--   nothing

select zip, regexp_substr(trim(zip) , '[0-9]{6}|[0-9]{5} |[0-9]{5}-|[0-9]{5}')
from tuition_stg
where regexp_like (trim(zip), '[0-9]{6}|[0-9]{5} |[0-9]{5}-|[0-9]{5}');
...
select zip
          , regexp_substr(trim(zip) , '^[0-9]{5}') mysubstr
          , case 
              when regexp_instr(trim(zip) , '^[0-9]{5}')>0
                  then substr(trim(zip) , 1, 5)
              when regexp_instr(trim(zip) , '^[0-9]{4}')>0
                  then substr(trim(zip) , 1, 4) || 0
             end  as  zip5  
           , case 
               when regexp_instr(trim(zip) , '^[0-9]{5}$')=1
                  or regexp_instr(trim(zip) , '^[0-9]{4}$')=1
                 then null
               when regexp_instr(trim(zip) , '( |-)[0-9]{3}$')>0
                  then regexp_substr(trim(zip) , '[0-9]{3}$') || '0'
               when regexp_instr(trim(zip) , '^[0-9]{9}$|^[0-9]{5} |^[0-9]{4}')=1
                  then regexp_substr(trim(zip),'[0-9]{4}$')
    end as zip4     
from tuition_stg;


select 'Humuhumunukunukuapuaa',regexp_count('Humuhumunukunukuapuaa', 'u(h|k|a)')
from dual;


select regexp_substr('h+ello world','[[:alpha:]+]')
from dual;

select regexp_replace(' when the company Codd Cruises Lines (CCLX, 24.74) was ', 'company') as press_release
from dual;
