...
commit;
  ...
  AND ship_id = 3;
  SELECT * FROM cruise_customers;
  INSERT INTO criuses_customers VALUES
    (10, 'Buffy', 'Worthington'
    );
  SELECT vendor_name,
  ...
  FROM books b
  WHERE retail >= ALL
    (SELECT AVG(retail) FROM books WHERE category = b.category
    )
  ORDER BY isbn;
  UPDATE books b
...
WHERE shipdate-orderdate =
  (SELECT shipdate-orderdate FROM orders WHERE shipdate IS NOT NULL
  );
SELECT customer#,
  city,
  state,
...
BEGIN
  INSERT INTO cruise_orders2 VALUES
    (1,22,sysdate
    );
  INSERT INTO cruise_orders2 VALUES
    (2,10,sysdate
...
ALTER TABLE cruise_orders2 ADD order_date DATE;
-- Page 432
-- cannot drp a column if it is referenced by a FK in anotehr table
-- Page 433
...
ALTER TABLE junk RENAME CONSTRAINT SYS_C004153 TO nn_mynew;
...
CREATE TABLE invoices4
  (
    ...
    (CREATE INDEX ix_invoices4 ON invoices4
      (invoice_id
      )
    ),
    invoice_date DATE
...
CREATE TABLE invoices5
  (
    ...
    (CREATE INDEX ix_invoices5 ON invoices5
      (invoice_id
      )
    )
  );
...
SELECT * FROM customers2 WHERE (upper(last_name)) = 'SMITH';
...
SELECT * FROM recyclebin;
SELECT scn_to_timestamp(667154) FROM dual;
...
AS
  ...
  CREATE TABLE invoices_external
    ...
    (
      type oracle_loader DEFAULT directory bank_files access parameters (records delimited BY newline skip 2 fields ( invoice_id CHAR(6), invoice_date CHAR(13), invoice_amt CHAR(9), account_number CHAR(11)))
...
AND ship_cabin_id   > 3
ORDER BY 1;
SELECT * FROM ship_cabins WHERE ship_cabin_id < 7 AND ship_cabin_id > 3;
SELECT room_type,
  SUM(sq_ft)
FROM ship_cabins
WHERE ship_cabin_id < 7
...
ORDER BY 1,
  2,
  3;
SELECT * FROM ship_cabins WHERE ship_cabin_id < 12 AND ship_cabin_id > 7;
-- -----------------------------------------------------------------------------
-- CUBE 1 C O L U M N
-- -----------------------------------------------------------------------------
-- find the same rows
...
ORDER BY room_style,
  room_type;
SELECT NVL(DECODE (grouping(room_type),1, upper(room_style),initcap(room_style)),'GRAND TOTAL') room_style,
  NVL(room_type,'Sub Total') room_type,
  SUM(sq_ft) sum_sq_ft
FROM ship_cabins
WHERE ship_id = 1
GROUP BY rollup(room_style, room_type)
...
IS
  ...
  SELECT COUNT(*),
    COUNT(
    CASE
      WHEN TO_CHAR(invoice_date,'RRRR') < '2010'
      THEN 1
    END) AS Y09,
    COUNT(
    CASE
...
CREATE sequence seq_cruise_customer_id start with 5;
  ...
  SET
    ...
    =
    (SELECT NVL(COUNT(s.ship_id),0) total,
      NVL(SUM(s.capacity),0) capacity
    FROM ships s
    WHERE s.home_port_id = p.port_id
    GROUP BY home_port_id
    );
  SELECT home_port_id,
    NVL(COUNT(s.ship_id),0) total,
    NVL(SUM(s.capacity),0) capacity
  FROM ships s
  GROUP BY home_port_id;
  -- ----------------------------------------------------------------
  ...
  INSERT WHEN (invoice_date > (add_months(sysdate,-18))) THEN
  INTO invoices_revised_archive
    (
      invoice_id,
      invoice_date,
...
BEGIN
  INSERT INTO ship_cabin_grid VALUES
    ('ROYAL', 1745,1635, NULL
    );
  INSERT INTO ship_cabin_grid VALUES
    ('SKYLOFT', 722,72235, NULL
    );
  INSERT INTO ship_cabin_grid VALUES
...
WHEN matched THEN
  ...
  DELETE WHERE wwa.inv_date < to_date('01-SEP-09') WHEN NOT matched THEN
  INSERT
    (
      wwa.inv_id,
      wwa.cust_po,
...
CREATE sequence seq_chat_id;
  BEGIN
    INSERT INTO chat VALUES
      (seq_chat_id.nextval, USER, 'Hi there.'
      );
    INSERT
    INTO chat VALUES
      (
    ...
    INSERT INTO chat VALUES
      (seq_chat_id.nextval,USER,'Online order form is up.'
      );
    INSERT INTO chat VALUES
      (seq_chat_id.nextval, USER, 'Over and out.'
      );
    COMMIT;
  END;
  /
  ...
  SELECT level,
    employee_id,
    title,
    connect_by_root title AS root_ancestor
  FROM employee_chart
  ...
  -- --------------------------------------------------------
  SELECT park_name,
    regexp_substr(park_name, 'State Park')
  FROM park;
  -- --------------------------------------------------------
  ...
  -- ------------------------------------------------
  SELECT regexp_instr('But, soft! What light through yonder window breaks?' ,'o[[:alpha:]]{1,2}', 1, 3) AS result
  FROM dual;
  -- ------------------------------------------------
  ...
  FROM dual;
  SELECT regexp_substr('123 Maple Avenue', '[[:alnum:] ]+',5,1) address
  FROM dual;
  -- ------------------------------------------------
  -- page 648 middle
  SELECT address2,
  ...
  SELECT address2,
    regexp_substr(address2,'74$') last_digit
  FROM ORDER_ADDRESSES
  WHERE regexp_like(address2,'74$');
  -- ------------------------------------------------
  SELECT address2,
  ...
  CREATE TABLE email_list
    (
      ...
      email1        VARCHAR2(120),
      CONSTRAINT ck_el_email1 CHECK ( regexp_like (email1, '^([[:alnum:]]+)@[[:alnum:]]+.(com|net|org|edu|gov|mil)$') )
    );
