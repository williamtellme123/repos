/************************************************************
**************Reliability Avg and Trend - 180 Day***********
************************************************************/
...
--TOTAL CALC:
select Avg(100 - (((DOWN_TIME *24)/ total_circuits)*100)) as TotalNetworkHealth
from
(select day, sum(downtime) down_time, sum(downdeadtime) down_dead_time
  ...
  and day between trunc(sysdate) - 179 and trunc(sysdate)
...
select Avg(100 - (((DOWN_TIME + DOWN_DEAD_TIME)/ total_circuits)*100)) as NetworkHealth
from
(select day, sum(downtime) down_time, sum(downdeadtime) down_dead_time
