...
select start_date from migration_stg;  
...
-- Display all information for devices in both legacy and next_gen
    select *
    from legacy L join next_gen NG on L.mac_add_short = NG.mac_short;


-- Display all information for devices in legacy but not yet in next_gen
      select *
      from legacy L left join next_gen NG on L.mac_add_short = NG.mac_short
      where ng.mac_short is null;
      
-- Display all information for new devices never in legacy but now in next_gen
      select *
      from legacy L right join next_gen NG on L.mac_add_short = NG.mac_short
      where l.mac_add is null; 

-- Based on average time to move a device how much longer before the project is complete
-- if each device is moved by one person and must be complete before the next device can bge moved


select floor(end_date-start_date) || ' DAYS ' ||
    mod(floor((end_date-start_date)*24),24) || ' HOURS ' ||
    mod(floor((end_date-start_date)*24*60),60)|| ' MINUTES ' ||
    mod(floor((end_date-start_date)*24*60*60),60) || ' SECONDS'
from migration;

select 
    floor(tot) || ' DAYS ' ||
    mod(floor((tot)*24),24) || ' HOURS ' ||
    mod(floor((tot)*24*60),60)|| ' MINUTES ' ||
    mod(floor((tot)*24*60*60),60) || ' SECONDS'
from 
    (
      select 
            (
              select  
                     avg (end_date-start_date) atime
              from migration
            ) *  
            ( 
              select count(*)
      ...
      from dual) 
;
-- 7 DAYS 14 HOURS 0 MINUTES 42 SECONDS
...
,to_flr
,to_rm
,to_char(start_date,'dd-MON-yyyy hh:mi:ss pm')
,to_char(end_date,'dd-MON-yyyy hh:mi:ss pm')
