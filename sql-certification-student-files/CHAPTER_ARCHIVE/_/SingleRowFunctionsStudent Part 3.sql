...
--    1.      
      ...
      desc some_numbers_stg;
      select to_number('3456'), to_number('3456') + 2, '123' + 5 from dual;
      select to_number(monthly_sales, '$999,999.99') from some_numbers_stg; 
      
      -- ETL
      desc some_numbers_final;
      insert into some_numbers_final values (1234567.12345);
      insert into some_numbers_final values (9999999999.994);
      insert into some_numbers_final values (9999999999.995);
      ...
      select monthly_sales from some_numbers_final;
--    2. 
...
--    4. converts char, varchar2, nchar, or nvarchar2 datatype to an interval day to second type.
      select * from some_hiredates_stg;
      insert into some_hiredates_stg values (9,'Bob2', to_date('01-JAN-1969', 'dd-MON-yy'));
      
      select hiredate from some_hiredates_stg where empo_id = 8; 
      update some_hiredates_stg
             set hiredate = to_date('01-JAN-69', 'dd-MON-rr')
             where empo_id = 8;
      
      select to_char(hiredate,'dd-MON-yyyy') from some_hiredates_stg where empo_id = 8;
...
--    TO_TIMESTAMP_TZ converts a TIMESTAMP  to a TIMESTAMP WITH LOCAL TIME ZONE value
--    CAST
--    EXTRACT
--    SYS_EXTRACT_UTC
--  1. 
...
--  Extracts value: YEAR, MONTH, DAY, HOUR, MINUTE, SECOND
    select extract(minute from to_timestamp('2009-10-11 12:13:14', 'yy-mm-dd hh24:mi:ss')) minute
    from dual;
-- 4.
    select extract(hour from to_timestamp('2009-10-11 12:13:14', 'yy-mm-dd hh24:mi:ss')) as "hour"
    from dual;
-- 5. 
    select extract(day from to_timestamp('2009-10-11 12:13:14', 'yyyy-mm-dd hh24:mi:ss')) as day
    from dual;
-- 6.    
...
--  Converts the source data into the local time equivalent
    select from_tz(
                    cast(
                            to_date('1999-12-01 11:00:00','rrrr-mm-dd hh:mi:ss') as timestamp)
                            ,
                            'America/Los_Angeles') at local 
                    as east_coast_time
    from dual; 
  
  select '123' + 5 from dual;
  select to_date('01-JAN-16', 'dd-MON-yy') + 5 from dual;
  select months_between('01-APR-12', last_day('15-JAN-12')+1) from dual;
...
-- Question 14
    ...
    select extract(timezone_region from email_received) 
          , extract(timezone_region from email_sent)from email_response;    