...
--    Hint:
--    1. On Page 172 Table 5-1 write note "Compare Page 353-4 Table 9-1"
...
-- c. AND and OR 
-- -----------------------------
-- family life and computer books btw 40-60
-- and executes before the or
Select category, title, retail, cost
from books 
where 
      retail >= 10 and retail <=60
...
from vendors 
where lower(category) like '%supplier%'
or lower(category) like '%subcontractor%'
or lower(category) like '%partner';


select table_name
from all_tab_columns
where lower(column_name) like '%vendor_name%';

select column_name
from all_tab_columns
where lower(table_name) like '%vendors%';
...
from ships
where 1=1 
  and ship_name in ('Codd Elegance','Codd Victorious')
  and lifeboats <= 100 and lifeboats >= 80
  and capacity / lifeboats > 25
;
...
-- alter each tabler to add a primary key 
--    customers :: (customer#)
--    orders :: (order#)
...
-- alter the following tables to add these foreign keys
--    orders :: cust_fk (customer#) to customers(customer#)
--    orderitems :: ord_fk (order#) references orders(order#)
--    orderitems ::  bk_fk (isbn) references books(isbn)
--    bookauthor ::  books_fk (isbn) references books(isbn)
--    bookauthor ::  auth_fk (authorid) references author(authorid)
--    books :: pub_fk(pubid) references publisher(pubid)
...
-- Now we will create the ER Diagram together

-- --------------------------------------------
-- 7. Review Learning Joins : Introduction
-- --------------------------------------------
/*
  ...
  But this will get you caught up to others for now.
*/
-- Table Creation
...
commit;
-- -----------------------------------------------------------------------------
-- The cartesean product or cross join
...
--      Movies
--        movie_id (integer-pk)
--        movie_name (50 characters)
--      Roles
--        role_id(integer-pk)
--        aid(foreign key to movies)
--        mid(foreign key to actors)
--        role_type (check constraint "Lead", "Supporting, "Cameo")
--      Actors
--        actor_id (integer-pk)
--        actor_fname(50 characters)
--        actor_lname(50 characters)
-- INSERT JAWS into Movies
-- INSERT "LEAD into Roles"
-- INSERT Actor First Name as "The"
-- INSERT Actor Last Name as "Shark"
-- 1. create the 1st parent Movies
    create table movies
    (movie_id integer primary key,
    movie_name varchar2(50));
-- 2. create the 2nd parent Actors
    create table actors
...
-- 3. create the child
    ...
    (   role_id integer primary key
      ...
      , role_type varchar2(15)
        ...
        , constraint actor_fk foreign key(aid) references actors(actor_id)
        , constraint role_ck check (role_type in ('Lead', 'Supporting', 'Cameo'))
      );
-- 4. Inerts into both parents first
    insert into movies values(100,'Jaws');
     insert into movies values(200,'Jaws');
      insert into movies values(300,'Jaws');
       insert into movies values(400,'Jaws');
    
    
    insert into actors values(2312, 'The','Shark');
-- 5. Inerts into child
    insert into roles values (27, 100, 2312, 'Lead');
