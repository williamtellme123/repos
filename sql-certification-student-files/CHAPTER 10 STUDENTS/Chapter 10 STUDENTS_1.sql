...
-- -----------------------------------------------------------------------------
    ...
    -- -------------------------------  
        ...
        update vw_books
        set retail = 1.99 where title =  'COOKING WITH MUSHROOMS';
        select * from vw_books;
        select * from books;
    -- -------------------------------      
        ...
        insert into books values ('512121212','JUMP UPSTREAM WITH SQL','21-AUG-01',4,22.75,32.95,'FAMILY LIFE');
        select * from vw_books;
        select * from books;
    ...
    -- -------------------------------  
        ...
        select * from vw_books where title = 'BUMBERS 2';
        select * from books where title = 'BUMBERS';
    
    -- -------------------------------  
        -- Can you update data in table outisde of view (aka write)?
        -- INSERT
        insert into vw_books values (666666, 'All Sixes', 55.55, 123.45, 'CARS');
        -- UPDATE
        update vw_books set retail = 99 where isbn = '666666';
        -- DELETE
        delete vw_books where isbn = '666666';
    ...
    -- -------------------------------         
        ...
        insert into vw_books (isbn, title, retail, cost, category) 
                       values('223999', 'SPUDS',25.99,19.99,'COOKING');
        select * from vw_books;
        select * from books;
    -- -------------------------------         
...
-- 2. Cannot insert unless view holds primary key, and follows all other contraints
...
--  Sequence 5
    drop sequence myseq;
    create sequence myseq start with 21 
                    cache 6 increment by 2 maxvalue 33 cycle;
    select myseq.nextval from dual;
...
--  Sequence 6
    ...
    -- will this work?
    create sequence myseq start with 21 
                          cache 6 increment by 2 
                          maxvalue 3 cycle minvalue 15;
    select myseq.nextval from dual;
...
--  Sequence 7
      drop sequence seq1;
      create sequence seq1 start with 150 
            cache 4 increment by 10 cycle 
            minvalue 150 maxvalue 190;
            
            
      drop sequence seq2;      
      create sequence seq2 start with 127 
            cache 4 increment by 5 cycle 
            maxvalue 147;
      ...
      drop sequence myseq;
      create sequence myseq start with 5 cache 6 
          increment by -2 cycle minvalue -6 maxvalue 7;
      select myseq.nextval from dual;
      
      drop sequence myseq;
      create sequence myseq start with -3 
              cache 4 increment by -2 
              cycle minvalue -10;
      select myseq.nextval from dual;
...
-- =============================================================================
    ...
    create table mystates as select * from states;
    create index ix_state on mystates(region, st);
    -- ------------------------------------           
    -- p. Check the index 
    select * from all_indexes
    where table_owner in ('BILLY')
    and table_name = 'MYSTATES';
