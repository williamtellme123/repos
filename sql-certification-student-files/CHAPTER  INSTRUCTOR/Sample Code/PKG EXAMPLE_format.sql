...
-- basic pacakge body
-- user defined types
CREATE OR REPLACE PROCECURE
PACKAGE BODY DASHBOARD.metric_rollups_dtv
AS
      -- -----------------------------------------------------------------------
      PROCEDURE driver_dtv(
      ...
      IS
        ...
        CURSOR period_cur ( i_last_ran_date IN DATE )
        IS
          SELECT period_id,
            period_name,
            period_type,
            ...
            end_date
          FROM rdr_period
          WHERE end_date >= TO_DATE ( TO_CHAR (TRUNC (i_last_ran_date - 1), 'MM/DD/YYYY')
            || '11:59:59 PM', 'MM/DD/YYYY HH:MI:SS AM' )
          AND end_date <= TO_DATE ( TO_CHAR (TRUNC (SYSDATE - 1), 'MM/DD/YYYY')
            || '11:59:59 PM', 'MM/DD/YYYY HH:MI:SS AM' );
      BEGIN
        metric_utilities.log_writer ('ALL', 'DTV METRIC_ROLLUP DRIVER START', SYSDATE, NULL);
        -- -----------------------------------------------------------------------------------------
        -- Get last run date get_im_stage_dtv
        metric_utilities.determine_last_ran ('IM - DTV Load Stage', 'M', v_im_stage_dtv_last_ran_date, v_im_stage_dtv_last_ran_status);
        v_im_stage_dtv_last_ran_date := v_im_stage_dtv_last_ran_date + 1 / 86400;
        metric_utilities.determine_last_ran ('IM - DTV Load History', 'M', v_im_hist_dtv_last_ran_date, v_im_hist_dtv_last_ran_status);
        v_im_hist_dtv_last_ran_date := v_im_hist_dtv_last_ran_date + 1 / 86400;
        metric_utilities.determine_last_ran ('IM - DTV All Period Rollups', 'M', v_im_ru_dtv_last_ran_date, v_im_ru_dtv_last_ran_status);
        v_im_ru_dtv_last_ran_date := v_im_ru_dtv_last_ran_date + 1 / 86400;
        -- -----------------------------------------------------------------------------------------
        metric_utilities.determine_last_ran ('PM - DTV Pull', 'M', v_pm_pull_last_ran, v_pm_status);
        metric_utilities.determine_last_ran ('PM - DTV Rollup', 'M', v_pm_ru_last_ran, v_pm_status);
        metric_utilities.determine_last_ran ('CM - DTV Pull', 'M', v_cm_pull_last_ran, v_cm_status);
        v_cm_pull_last_ran := v_cm_pull_last_ran + 1 / 86400;
        metric_utilities.determine_last_ran ('CM - DTV Rollup', 'M', v_cm_ru_last_ran, v_cm_status);
        v_cm_ru_last_ran := v_cm_ru_last_ran + 1 / 86400;
        -- -------------------------------------------------------------------------------------------
        -- Check DTV link
        BEGIN
          SELECT SYSDATE INTO v_dtv_dual_date FROM DUAL@DTVREMP1.CISCO.COM hd;
          -- handles remaining exceptions not handled by Named System Exceptions or Ueser Exceptions
        EXCEPTION
        WHEN OTHERS THEN
          v_dtv_dual_date := NULL;
          v_msg           := SQLERRM;
          metric_utilities.log_writer ( 'ALL', 'DTV METRIC_ROLLUP DRIVER ERROR - dtv link validation', SYSDATE, v_msg );
        END;
        IF v_dtv_dual_date     IS NOT NULL THEN
        ...
        metric_utilities.log_writer ( 'ALL', 'METRIC_ROLLUP DTV Connection status ' || v_dtv_connect_status, SYSDATE, NULL );
        -- -------------------------------------------------------------------------------------------
        -- LOAD IM_STAGE_DATA
        IF v_dtv_connect_status = 'SUCCESS' THEN
          INSERT
          INTO metric_control
            (
              metric_name,
        ...
        IF v_dtv_connect_status = 'SUCCESS' THEN
          im_get_data_dtv ( v_im_stage_dtv_last_ran_date, TO_DATE (TO_CHAR (SYSDATE - 1, 'MM/DD/YYYY') || '11:59:59 PM', 'MM/DD/YYYY HH:MI:SS AM'), v_im_stage_status_dtv );
          metric_utilities.log_writer ('ALL', 'IM - DTV Load Stage Complete', SYSDATE, NULL);
        ELSE
          metric_utilities.set_current_run_stop ('IM - DTV Load Stage', v_dtv_connect_status);
        END IF;
        metric_utilities.set_current_run_stop ('IM - DTV Load Stage', v_im_stage_status_dtv);
        -- -------------------------------------------------------------------------------------------
        -- LOAD IM_HISTORY_DATA
        -- -----------------------------------------------------------------------------------------
        -- If stage table loaded then update im_history_data
        IF v_im_stage_status_dtv = 'SUCCESS' THEN
          INSERT
          INTO metric_control
            (
              metric_name,
        ...
        IF v_im_stage_status_dtv = 'SUCCESS' THEN
          ...
          metric_utilities.log_writer ('ALL', 'IM - DTV Load History Complete', SYSDATE, NULL);
        ELSE
          metric_utilities.set_current_run_stop ('IM - DTV Load History', v_im_hist_status_dtv);
        END IF;
        metric_utilities.set_current_run_stop ('IM - DTV Load History', v_im_hist_status_dtv);
        -- ----------------------------------------------------------------------------
        ...
        IF v_im_hist_status_dtv = 'SUCCESS' THEN
          -- Log the start of All Period Rollups Startting
          metric_utilities.log_writer ('ALL', 'IM - DTV All Period Rollups Starting', SYSDATE, NULL);
          ...
          LOOP
            ...
            im_rollup_data_dtv (period_rec.start_date, period_rec.end_date, period_rec.period_id, period_rec.period_type, period_rec.period_name, v_im_rollup_status_dtv);
            -- set the stop with the status returned
            metric_utilities.set_current_run_stop ('IM - DTV Rollup', v_im_rollup_status_dtv);
            metric_utilities.log_writer ( 'ALL', 'IM_DTV:' || period_rec.period_type || ' is Complete - Period: ' || period_rec.period_id, SYSDATE, NULL );
          END LOOP;
          -- Log the end of all DTV Rollups
          metric_utilities.log_writer ('ALL', 'IM - DTV All Period Rollups Complete', SYSDATE, NULL);
          -- Set the metric_control entry for all with the status of the last period status
          metric_utilities.set_current_run_stop ( 'IM - DTV All Period Rollups', v_im_rollup_status_dtv );
        ELSE
          INSERT
          INTO metric_control
            (
          ...
          metric_utilities.log_writer ('ALL', 'METRIC_ROLLUP_DTV IM Failed', SYSDATE, NULL);
        END IF;
        -- =========================================================================================
        -- Do PM
        -- =========================================================================================
        metric_utilities.log_writer ('ALL', 'PM - DTV Pull Starting ' || v_pm_status, SYSDATE, NULL);
        IF v_pm_status = 'SUCCESS' THEN
          INSERT
          INTO metric_control
            (
              metric_name,
        ...
        END IF;
        IF v_pm_status = 'SUCCESS' THEN
          pm_get_data_dtv ( v_pm_pull_last_ran, TO_DATE (TO_CHAR (SYSDATE - 1, 'MM/DD/YYYY') || '11:59:59 PM', 'MM/DD/YYYY HH:MI:SS AM'), v_pm_status );
        ELSE
          metric_utilities.set_current_run_stop ('PM - DTV Pull', v_pm_status);
        END IF;
        metric_utilities.log_writer ( 'ALL', 'PM - DTV Update Starting ' || v_pm_status, SYSDATE, NULL );
        IF v_pm_status = 'SUCCESS' THEN
        ...
        metric_utilities.log_writer ( 'ALL', 'PM - DTV Rollup Starting ' || v_pm_status, SYSDATE, NULL );
        IF v_pm_status = 'SUCCESS' THEN
          INSERT
          INTO metric_control
            (
        ...
        --Do CM
        --Check for successful determination of last run date AND THEN proceed
        metric_utilities.log_writer ( 'ALL', 'CM - DTV Get Data Starting ' || v_cm_status, SYSDATE, NULL );
        IF v_cm_status = 'SUCCESS' THEN
          INSERT
          INTO metric_control
            (
              metric_name,
        ...
        END IF;
        IF v_cm_status = 'SUCCESS' THEN
          cm_get_data_dtv ( v_cm_pull_last_ran, TO_DATE (TO_CHAR (SYSDATE - 1, 'MM/DD/YYYY') || '11:59:59 PM', 'MM/DD/YYYY HH:MI:SS AM'), v_cm_status );
        ELSE
          metric_utilities.set_current_run_stop ('CM - DTV Pull', v_cm_status);
        END IF;
        metric_utilities.log_writer ( 'ALL', 'CM - DTV Update Starting ' || v_cm_status, SYSDATE, NULL );
        IF v_cm_status = 'SUCCESS' THEN
        ...
        END IF;
        metric_utilities.set_current_run_stop ('CM - DTV Pull', v_cm_status);
        metric_utilities.log_writer ( 'ALL', 'CM - DTV Rollup Starting ' || v_cm_status, SYSDATE, NULL );
        IF v_cm_status = 'SUCCESS' THEN
          INSERT
          INTO metric_control
            (
        ...
        END IF;
        metric_utilities.set_current_run_stop ('CM - DTV Rollup', v_cm_status);
        metric_utilities.log_writer ('ALL', 'METRIC_ROLLUP DRIVER COMPLETE', SYSDATE, NULL);
      EXCEPTION
      WHEN OTHERS THEN
        v_msg := SQLERRM;
        metric_utilities.log_writer ('ALL', 'METRIC_ROLLUP DRIVER ERROR', SYSDATE, v_msg);
      END driver_dtv;
      -- ----------------------------------------------------------------------------------------------
      -- INCIDENT MANAGEMENT
      -- ----------------------------------------------------------------------------------------------
      --Gets stage data FROM Incident Managment bASe tables
      PROCEDURE im_get_data_DTV
        (
          i_start_date IN DATE ,
          i_stop_date  IN DATE ,
          o_im_status OUT VARCHAR
        )
      IS
      ...
      IS
        ...
        INDEX BY BINARY_INTEGER;
        -- 3. alarm_to_analyze
      TYPE in_progress_date_list
      IS
        TABLE OF DATE INDEX BY BINARY_INTEGER;
      TYPE reported_date_list
      IS
        TABLE OF DATE INDEX BY BINARY_INTEGER;
        -- 4. inc_mttc
        -- 6. met_mttc sla
      TYPE alarm_to_crt_mins_list
      IS
        TABLE OF NUMBER INDEX BY BINARY_INTEGER;
        -- 5. inc_mttn
        -- 7. mttn_sla_met
      TYPE tkt_create_to_notify_mins_list
      IS
        TABLE OF NUMBER INDEX BY BINARY_INTEGER;
      TYPE submitter_list
      IS
      ...
      IS
        TABLE OF NUMBER INDEX BY BINARY_INTEGER;
      TYPE reported_network_condition_tbl
      IS
        TABLE OF NUMBER INDEX BY BINARY_INTEGER;
      TYPE tkt_create_to_resolved_hrs
      IS
        TABLE OF NUMBER INDEX BY BINARY_INTEGER;
        -- ---------------------------------------------------------------------------
      ...
      BEGIN
        ...
        -- Determine which incident numbers to pull
        INSERT
        INTO im_stage_ids_to_pull_work_dtv
          (
            incident_number
          )
        ...
        COMMIT;
        INSERT INTO im_stage_data_ids_to_pull_dtv
          (incident_number
          )
        ...
        --Populate stage data
        INSERT
        INTO DASHBOARD.IM_STAGE_DATA_DTV
          (
            incident_number ,
            alarm_identifier ,
            alarm_type ,
        ...
        SELECT
          ...
          ,
          CASE
            WHEN hd.reported_network_condition = 0
            THEN 3
            WHEN hd.reported_network_condition = 1
            THEN 2
            WHEN hd.reported_network_condition = 2
            THEN 1
            WHEN hd.reported_network_condition IS NULL
            THEN hd.priority
          END AS priority ,
          ...
          hd.site_id ,
          hd.state_province ,
          DECODE (hd.status, 0, 'New', 1, 'ASsigned', 2, 'In Progress', 3, 'Pending', 4, 'Resolved', 5, 'Closed', 6, 'Cancelled') AS status ,
          hd.status_reASon ,
          hd.submitter ,
          ...
          --in_progress_date
          ,
          TRUNC(metric_utilities.convert_to_date(hd.closed_date),'DD') AS day_closed ,
          TRUNC(metric_utilities.convert_to_date(hd.submit_date),'DD') AS day_opened ,
          TRUNC(metric_utilities.convert_to_date(hd.closed_date),'MM') AS month_closed ,
          TRUNC(metric_utilities.convert_to_date(hd.submit_date),'MM') AS month_opened ,
          TRUNC(metric_utilities.convert_to_date(hd.closed_date),'WW') AS week_closed ,
          TRUNC(metric_utilities.convert_to_date(hd.submit_date),'WW') AS week_opened
          -- Calculated Flags
          --            , tmp.auto_notify auto_notify_flag
          ,
          CASE
            WHEN hd.submitter = 'WEBSERVICES'
            THEN 'Y'
            ELSE 'N'
          END AS auto_tkt_flag ,
          CASE
            WHEN hd.status                    = 5
            AND hd.resolution_category_tier_2 = 'Duplicate Ticket'
            THEN 'Y'
            ELSE 'N'
          END AS closed_AS_duplicate_flag ,
          CASE
            WHEN hd.status             = 5
            AND hd.resolution_category = 'ROS InfrAStructure'
            THEN 'Y'
            ELSE 'N'
          END AS closed_AS_ros_infrAStr_flag ,
          CASE
            WHEN hd.status       = 5
            AND hd.status_reASon = 1500
            THEN 'Y'
            ELSE 'N'
          END AS closed_AS_merged_flag ,
          CASE
            WHEN hd.resolution_category_tier_2 = 'Scheduled Maint'
            THEN 'Y'
            ELSE 'N'
          END AS sched_maint_flag
          -- Calculated Counts
          ,
          NVL(
          ...
          -- time_to_notify        initial_notification_date - reported_date
          ,
          (metric_utilities.convert_to_date(hd.initial_notification_date) - metric_utilities.convert_to_date(hd.reported_date))*(24*60) alarm_to_notify_mins
          ...
          -- time_to_isolate        isolation_date - reported_date
          ,
          (metric_utilities.convert_to_date(hd.isolation_date) - metric_utilities.convert_to_date(hd.reported_date))*(24*60) alarm_to_isolate_mins
          ...
          ,
          CASE
            WHEN hd.lASt_resolved_date IS NOT NULL
            AND hd.reported_date       IS NOT NULL
            AND hd.lASt_resolved_date   > hd.reported_date
            AND hd.status                                                IN (4,5)
            THEN (metric_utilities.convert_to_date(hd.lASt_resolved_date) - metric_utilities.convert_to_date(hd.reported_date))*(24*60)
            WHEN hd.lASt_resolved_date IS NULL
            AND hd.closed_date         IS NOT NULL
            AND hd.reported_date       IS NOT NULL
            AND hd.closed_date          > hd.reported_date
            AND hd.status                                         IN (4,5)
            THEN (metric_utilities.convert_to_date(hd.closed_date) - metric_utilities.convert_to_date(hd.reported_date))*(24*60)
          END AS alarm_to_resolve_mins
          ...
          -- time_to_close        close_date - reported_date
          ,
          (metric_utilities.convert_to_date(hd.closed_date) - metric_utilities.convert_to_date(hd.reported_date))*(24*60) alarm_to_close_mins
          ...
          ,
          (metric_utilities.convert_to_date(hd.submit_date) - metric_utilities.convert_to_date(hd.reported_date))*(24*60) alarm_to_create_mins
          ...
          CASE
            ...
            AND hd.status                                                IN (4,5)
            THEN (metric_utilities.convert_to_date(hd.lASt_resolved_date) - metric_utilities.convert_to_date(hd.submit_date))*(24)
            WHEN hd.lASt_resolved_date IS NULL
            AND hd.closed_date         IS NOT NULL
            AND hd.submit_date         IS NOT NULL
            AND hd.closed_date          > hd.submit_date
            AND hd.status                                         IN (4,5)
            THEN (metric_utilities.convert_to_date(hd.closed_date) - metric_utilities.convert_to_date(hd.submit_date))*(24)
          END AS tkt_create_to_resolved_hrs
          ...
          CASE
            ...
            AND hd.status                                                IN (4,5)
            THEN (metric_utilities.convert_to_date(hd.lASt_resolved_date) - metric_utilities.convert_to_date(hd.isolation_date))*(24*60)
            WHEN hd.lASt_resolved_date IS NULL
            AND hd.closed_date         IS NOT NULL
            AND hd.isolation_date      IS NOT NULL
            AND hd.closed_date          > hd.isolation_date
            AND hd.status                                         IN (4,5)
            THEN (metric_utilities.convert_to_date(hd.closed_date) - metric_utilities.convert_to_date(hd.isolation_date))*(24*60)
          END AS tkt_isolate_to_resolved_mins
          ...
          END AS tkt_resolved_to_closed_mins ,
          'DTVREMP1.CISCO.COM'
          ...
          DECODE(hd.submitter, 'WEBSERVICES', 'Y', 'N') AS system_created ,
          v_lASt_updated
          -- REPORTED_NETWORK_CONDITION
        ...
        AND hd.incident_number IN
          (SELECT incident_number FROM im_stage_data_ids_to_pull_dtv
          )
        ...
        SELECT i.incident_number,
          NVL(COUNT(
          CASE
            WHEN t.tASkname = 'Initial Notification - Manual'
            THEN 1
          END),0) AS cnt BULK COLLECT
        INTO my_ticket_list_an,
          my_tASk_count_list
        ...
        SELECT
          ...
          im.tkt_create_to_resolved_hrs BULK COLLECT
        INTO
          -- 0. ticket_list
          my_ticket_list
          -- 1. company_id
        ...
        -- bulk collect cursor
        FOR i IN my_ticket_list.FIRST .. my_ticket_list.LAST
        LOOP
          -- 1. company_id
          v_company_id := NULL;
          -- 2. auto_notify_flag
          ...
          -- 1. company id
          CASE
          WHEN my_company_list(i) = 'DIRECTV-NGBN' THEN
            v_company_id         := 'CPY000000001337';
          WHEN my_company_list(i) = 'DIRECTV-BROADBAND' THEN
            v_company_id         := 'CPY000000001338';
          WHEN my_company_list(i) = 'DIRECTV-CONUS' THEN
            v_company_id         := 'CPY000000001339';
          WHEN my_company_list(i) = 'DIRECTV' THEN
            v_company_id         := '';
          END CASE;
          -- 2. autonotify
          ...
          END IF;
          -- 4. inc_mttcreate
          IF my_alarm_to_crt_mins_list(i) >= 0 AND my_alarm_to_crt_mins_list(i) < 100 AND my_submitter_list(i) = 'WEBSERVICES' THEN
            v_inc_mttcreate               := 'Y';
          ELSE
            v_inc_mttcreate:= 'N';
          END IF;
          -- 5. inc_mttn
          IF my_alarm_to_crt_mins_list(i) >= 0 AND my_alarm_to_crt_mins_list(i) < 100 AND v_auto_notify_flag = 'N' THEN
            v_inc_mttn                    := 'Y';
          ELSE
            v_inc_mttn := 'N';
          END IF;
          -- 6. MTTC determine if this record met MTTC SLA
          IF my_alarm_to_crt_mins_list(i) <= 7 AND v_inc_mttcreate = 'Y' THEN
            v_mttcreate_sla_met           := 'Y';
          ELSE
            v_mttcreate_sla_met := 'N';
          END IF;
          -- 7. met_mttn_sla
          IF my_tkt_create_to_not_mins_list(i) <= 15 AND v_inc_mttn = 'Y' THEN
            v_mttn_sla_met                     := 'Y';
          ELSE
            v_mttn_sla_met := 'N';
          END IF;
          ...
          -- determine if null
          IF my_reptd_network_cond_list(i) IS NULL THEN
            reported_net_cond_is_blank     := TRUE;
          END IF;
          -- ---------------------------------------------
          -- P1 met MTTR SLA
          IF NOT (reported_net_cond_is_blank) AND my_reptd_network_cond_list(i) = 2 AND my_tkt_create_to_res_hrs_list(i) <= 4 THEN
            v_mttr_p1_sla_met                                                  := 'Y';
          ELSE
            IF NOT (reported_net_cond_is_blank) AND my_reptd_network_cond_list(i)= 2 THEN
              v_mttr_p1_sla_met                                                 := 'N';
            ELSE
              v_mttr_p1_sla_met := NULL;
            END IF;
          END IF;
          -- --------------------------------------------
          IF reported_net_cond_is_blank AND my_priority_list(i) = 1 AND my_tkt_create_to_res_hrs_list(i) <= 4 THEN
            v_mttr_p1_sla_met                                  := 'Y';
          ELSE
            IF reported_net_cond_is_blank AND my_priority_list(i) = 1 THEN
              v_mttr_p1_sla_met                                  := 'N';
            ELSE
              v_mttr_p1_sla_met := NULL;
            END IF;
          ...
          -- determine if null
          -- P2 met MTTR SLA
          IF NOT (reported_net_cond_is_blank) AND my_reptd_network_cond_list(i) = 1 AND my_tkt_create_to_res_hrs_list(i) <= 4 THEN
            v_mttr_p2_sla_met                                                  := 'Y';
          ELSE
            IF NOT (reported_net_cond_is_blank) AND my_reptd_network_cond_list(i)= 1 THEN
              v_mttr_p2_sla_met                                                 := 'N';
            ELSE
              v_mttr_p2_sla_met := NULL;
            END IF;
          END IF;
          -- --------------------------------------------
          IF reported_net_cond_is_blank AND my_priority_list(i) = 2 AND my_tkt_create_to_res_hrs_list(i) <= 4 THEN
            v_mttr_p2_sla_met                                  := 'Y';
          ELSE
            IF reported_net_cond_is_blank AND my_priority_list(i) = 2 THEN
              v_mttr_p2_sla_met                                  := 'N';
            ELSE
              v_mttr_p2_sla_met := NULL;
            END IF;
          ...
          -- determine if null
          -- P2 met MTTR SLA
          IF NOT (reported_net_cond_is_blank) AND my_reptd_network_cond_list(i) = 0 AND my_tkt_create_to_res_hrs_list(i) <= 24 THEN
            v_mttr_p3_sla_met                                                  := 'Y';
          ELSE
            IF NOT (reported_net_cond_is_blank) AND my_reptd_network_cond_list(i)= 0 THEN
              v_mttr_p3_sla_met                                                 := 'N';
            ELSE
              v_mttr_p3_sla_met := NULL;
            END IF;
          END IF;
          -- --------------------------------------------
          IF reported_net_cond_is_blank AND my_priority_list(i) = 3 AND my_tkt_create_to_res_hrs_list(i) <= 24 THEN
            v_mttr_p3_sla_met                                  := 'Y';
          ELSE
            IF reported_net_cond_is_blank AND my_priority_list(i) = 3 THEN
              v_mttr_p3_sla_met                                  := 'N';
            ELSE
              v_mttr_p3_sla_met := NULL;
            END IF;
      ...
      END im_get_data_DTV;
      -- ----------------------------------------------------------------------- 
      PROCEDURE im_update_history_dtv(
          o_im_status OUT VARCHAR)
      IS
        v_msg VARCHAR2(1000);
      BEGIN
        ...
        DELETE
        FROM im_history_data_dtv
        WHERE incident_number IN
          (SELECT incident_number FROM im_stage_data_dtv
          );
        COMMIT;
        ---------------------------------------------------------------
        --Update the im_history table
        INSERT
        INTO im_history_data_dtv
          (
            incident_number ,
            alarm_identifier ,
            alarm_type ,
      ...
      PROCEDURE pm_get_data_dtv(
          i_start_date IN DATE ,
          i_stop_date  IN DATE ,
          o_pm_status OUT VARCHAR)
      IS
        -- ---------------------------------------------------------------
      ...
      BEGIN
        ...
        SELECT
          ...
          DECODE (ppi.investigation_status, 0, 'Draft', 1, 'Under Review', 2, 'Request For Authorization', 3, 'ASsigned', 4, 'Under Investigation', 5, 'Pending', 6, 'Completed', 7, 'Rejected', 8, 'Closed', 9, 'Cancelled') AS status,
          ppi.priority,
          DECODE (ppi.invesitgation_status_reASon, 1000, 'Known Error', 2000, 'Unresolveable', 3000, 'Solution DatabASe', 4000, 'Enhancement Request', 5000, 'Duplicate Investigation', 'None' ) AS status_reASon,
          ppi.company,
          ppi.site,
          ppi.ASsigned_group,
          ...
          -- calculated flags
          CASE
            WHEN ppi.investigation_status       = 8
            AND ppi.invesitgation_status_reASon = 5000
            THEN 'Y'
            ELSE 'N'
          END AS closed_AS_duplicate_flag,
        ...
        FROM aradmin.pbm_problem_investigation@DTVREMP1.CISCO.COM ppi,
          ...
          -- calc_mmttouch
          (
          SELECT piw.problem_investigation_id pbi_id,
            metric_utilities.convert_to_date(MAX(piw.submit_date)) lASt_touch_date,
          ...
          GROUP BY piw.problem_investigation_id
          ) calc_mmttouch,
          -- calc_ttc
          (
          SELECT ppi.problem_investigation_id pbi_id,
            metric_utilities.convert_to_date(h.t8) closed_date,
          ...
          WHERE ppi.sys_problem_investigation_id = h.entryid
          ) calc_ttc,
          -- calc_ttr
          (
          SELECT ppi.problem_investigation_id AS pbi_id,
            metric_utilities.convert_to_date(h.t6) resolved_date,
          ...
          (
          SELECT ppi.problem_investigation_id   AS pbi_id,
            COUNT(piw.problem_investigation_id) AS num_touches
          FROM aradmin.pbm_problem_investigation@DTVREMP1.CISCO.COM ppi,
            aradmin.pbm_investigation_worklog@DTVREMP1.CISCO.COM piw
          WHERE ppi.problem_investigation_id = piw.problem_investigation_id
          GROUP BY ppi.problem_investigation_id
          ) count_touches,
          -- count_escalations
          (
          SELECT ppi.problem_investigation_id pbi_id,
            COUNT(piw.problem_investigation_id) AS num_escalations
          FROM aradmin.pbm_problem_investigation@DTVREMP1.CISCO.COM ppi,
            aradmin.pbm_investigation_worklog@DTVREMP1.CISCO.COM piw
          WHERE ppi.problem_investigation_id = piw.problem_investigation_id
          ...
          SELECT pia.request_id02,
            NVL(COUNT(
            CASE
              WHEN SUBSTR(pia.request_id01,1,3) = 'CRQ'
              THEN 1
              ELSE NULL
            END),0) AS num_CRQ_ASsociations,
            NVL(COUNT(
            CASE
              WHEN SUBSTR(pia.request_id01,1,3) = 'INC'
              THEN 1
              ELSE NULL
            END),0) AS num_INC_ASsociations,
            NVL(COUNT(
            CASE
              WHEN SUBSTR(pia.request_id01,1,3) = 'SDB'
              THEN 1
              ELSE NULL
            END),0) AS num_SDB_ASsociations,
            NVL(COUNT(
            CASE
              WHEN SUBSTR(pia.request_id01,1,3) NOT IN ('CRQ','INC','SDB')
              THEN 1
              ELSE NULL
            END),0) AS num_other_ASsociations
          FROM aradmin.pbm_investigation_ASsociations@DTVREMP1.CISCO.COM pia
          GROUP BY pia.request_id02
          ) count_ASsociations
        WHERE 1                           =1
        ...
        AND ppi.problem_investigation_id IN
          (SELECT DISTINCT pbi_id FROM pm_ids_to_pull
          )
        ...
        DECLARE
          CURSOR pbi_ticket_cur
          IS
            SELECT pbi_id FROM pm_stage_data ORDER BY pbi_id;
          CURSOR pbi_ASsoc_cur(i_pbi_id IN VARCHAR2)
          IS
            SELECT form_name01 ,
              form_name02 ,
              request_id01 ,
        ...
        BEGIN
          -- for each record
          FOR pbi_ticket_rec IN pbi_ticket_cur
          LOOP
            -- grab all the ASsociations
            FOR pbi_ASsoc_rec IN pbi_ASsoc_cur(pbi_ticket_rec.pbi_id)
            LOOP
              ...
              IF pbi_ASsoc_rec.ASsociation_type01 = '31000' AND SUBSTR(pbi_ASsoc_rec.request_id01,1,3) = 'CRQ' THEN
                UPDATE pm_stage_data
        ...
        SELECT solution_databASe_id sdb_id,
          ...
          DECODE (sdb.status, 0, 'Proposed', 1, 'Enabled', 2, 'Offline', 3, 'Obsolete', 4, 'Archive', 5, 'Delete') AS status,
          'DTV'
        FROM aradmin.pbm_solution_databASe@DTVREMP1.CISCO.COM sdb
        WHERE sdb.solution_databASe_id IN
          (SELECT DISTINCT sdb_id FROM pm_sbd_ids_to_pull
          );
        o_pm_status := 'SUCCESS';
      EXCEPTION
      WHEN OTHERS THEN
      ...
      END pm_get_data_dtv;
                  -- -------------------------------------------------------------------------
                  PROCEDURE pm_update_history_dtv(
                      o_pm_status OUT VARCHAR)
                  IS
                    v_msg VARCHAR2(1000);
                  BEGIN
                    DELETE
                    FROM pm_history_data
                    WHERE data_source = 'DTV'
                    AND pbi_id       IN
                      (SELECT pbi_id FROM pm_stage_data
                      );
                    COMMIT;
                    -- ---------------------------------------------------------------
                    --    9. insert into pm_history
                    INSERT
                    INTO pm_history_data
                      (
                        pbi_id,
                        submit_date,
                        lASt_modified_date,
                    ...
                    DELETE
                    FROM pm_sdb_history_data
                    WHERE data_source = 'DTV'
                    AND sdb_id       IN
                      (SELECT sdb_id FROM pm_sdb_stage_data
                      );
                    -- ---------------------------------------------------------------
                    --    21. insert into sdb_history
                    INSERT
                    INTO pm_sdb_history_data
                      (
                        sdb_id,
                        submit_date,
                        company,
            ...
            PROCEDURE cm_get_data_dtv(
                i_start_date IN DATE ,
                i_stop_date  IN DATE ,
                o_cm_status OUT VARCHAR)
            IS
              v_msg VARCHAR2(1000);
            BEGIN
              ...
              SELECT
                ...
                FROM aradmin.H1200@DTVREMP1.CISCO.COM h
                WHERE h.entryid = cic.request_id
                ) ,'MONTH')                                                         AS month_completed_h ,
                TO_CHAR(metric_utilities.convert_to_date(cic.submit_date), 'MONTH') AS month_opened
                -- Calculated Flags
                ,
                ca_set_flag.flag AS crq_created_by_INC ,
                CASE
                  WHEN cic.change_type = 2000
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_type_is_change_flag ,
                CASE
                  WHEN cic.change_type      = 2000
                  AND change_request_status = 9
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_is_rejected_flag ,
                CASE
                  WHEN cic.change_request_status = 12
                  AND status_reASon              = 3000
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_cancelled_rescheduled_flag ,
                CASE
                  WHEN cic.change_timing = 1000
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_is_emergency_flag ,
                CASE
                  WHEN cic.change_request_status = 11
                  AND status_reASon              = 8000
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_is_backed_out_flag ,
                CASE
                  WHEN cic.change_request_status = 11
                  AND status_reASon              = 5000
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_is_successful_flag ,
                CASE
                  WHEN cic.change_timing = 2000
                  THEN 'Y'
                  ELSE 'N'
                END AS chg_is_expedited_flag
                -- Calculated Counts
                ,
                NVL(count_incidents.num_inc_FROM_this_CRQ,0) AS num_inc_FROM_this_CRQ
                -- Calculated Times
                ,
                (
                CASE
                  WHEN cic.change_request_status = 10
                  THEN metric_utilities.convert_to_date(cic.completed_date) - metric_utilities.convert_to_date(cic.submit_date)
                END) AS time_to_complete_days ,
                'DTV'
              FROM aradmin.chg_infrAStructure_change@DTVREMP1.CISCO.COM cic
                -- count_incidents created by this change
                ,
                (SELECT cic.infrAStructure_change_id AS infrAStructure_change_id,
                  COUNT(ca.request_id02)             AS num_inc_FROM_this_CRQ
                FROM aradmin.chg_infrAStructure_change@DTVREMP1.CISCO.COM cic,
                ...
                FROM aradmin.chg_infrAStructure_change@DTVREMP1.CISCO.COM cic,
                  (SELECT ca.request_id02,
                    ca.request_id01,
                    CASE
                      WHEN ca.ASsociation_type01      =31000
                      AND SUBSTR(ca.request_id01,1,3) = 'INC'
                      THEN 'Y'
                      ELSE 'N'
                    END flag
              ...
              AND cic.infrAStructure_change_id  IN
                (SELECT DISTINCT(request_id) FROM cm_change_id_list
                );
              COMMIT;
              o_cm_status := 'SUCCESS';
            EXCEPTION
            ...
            END cm_get_data_DTV;
      -- -------------------------------------------------------------------------
      PROCEDURE cm_update_history_dtv(
          o_cm_status OUT VARCHAR)
      IS
        v_msg VARCHAR2(1000);
      BEGIN
        ...
        DELETE
        FROM cm_history_data
        WHERE data_source             = 'DTV'
        AND infrAStructure_change_id IN
          (SELECT infrAStructure_change_id FROM cm_stage_data
          );
        COMMIT;
        INSERT
        INTO CM_HISTORY_DATA
          (
            -- ATOMIC FIELDS
            INFRASTRUCTURE_CHANGE_ID ,
            ASCHG ,
