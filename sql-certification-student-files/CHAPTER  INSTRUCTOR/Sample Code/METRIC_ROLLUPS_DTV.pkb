CREATE OR REPLACE PACKAGE BODY DASHBOARD."METRIC_ROLLUPS_DTV" 
AS
   ...
   IS
      ...
      IS
           SELECT   incident_number
             FROM   im_stage_data_ids_to_pull_dtv
   ...
   BEGIN
      ...
      METRIC_UTILITIES.TRUNC_TABLE (
         'dashboard.im_stage_ids_to_pull_work_dtv'
      );

      -- Get distinct ticket numbers
      METRIC_UTILITIES.TRUNC_TABLE (
         'dashboard.im_stage_data_ids_to_pull_dtv'
      );
      ...
      INSERT INTO im_stage_ids_to_pull_work_dtv (incident_number)
         SELECT   hd.incident_number
          ...
          WHERE   metric_utilities.convert_to_date (hd.last_modified_date) >
                     i_start_date
         UNION
         SELECT   request_id02
           FROM   aradmin.hpd_associations@DTVREMP1.CISCO.COM ha
          WHERE   metric_utilities.convert_to_date (ha.last_modified_date) >
                     i_start_date
         UNION
         SELECT   incident_number
           FROM   aradmin.hpd_worklog@DTVREMP1.CISCO.COM hw
          WHERE   metric_utilities.convert_to_date (hw.last_modified_date) >
                     i_start_date;
      ...
      INSERT INTO im_stage_data_ids_to_pull_dtv (incident_number)
         SELECT   DISTINCT incident_number
           FROM   im_stage_ids_to_pull_work_dtv;

      COMMIT;

      FOR tkt_rec IN tkt_cur
      LOOP
         BEGIN
            --Populate stage data
            INSERT INTO DASHBOARD.IM_STAGE_DATA_DTV (incident_number,
                                                     alarm_identifier,
                                                     alarm_type,
                                                     assigned_group,
                                                     ...
                                                     company-- set in the cursor below
                                                            --, companyid
                                                     ,
                                                     correlated_ci,
                                                     correlated_id,
                                                     ...
                                                     total_time_spent,
                                                     total_transfers,
                                                     total_pending_time_in_minutes,
                                                     urgency-- Atomic Dates
                                                     ,
                                                     closed_date,
                                                     ...
                                                     responded_date,
                                                     submit_date,
                                                     service_restored_date-- Calculated Dates
                                                     ,
                                                     in_progress_date,
                                                     day_closed,
                                                     day_opened,
                                                     ...
                                                     sched_maint_flag-- Calculated Counts
                                                     ,
                                                     ...
                                                     num_touches-- no off_clock_time_mins possible with DTV Remedy
                                                                -- , off_clock_time_mins
                                                     ,
                                                     ...
                                                     alarm_to_close_mins,
                                                     alarm_to_serv_restor_mins-- Set in the cursor below
                                                     ...
                                                     Tkt_Create_to_serv_restor_mins,
                                                     Tkt_Isolate_To_Resolved_Mins,
                                                     Tkt_Resolved_To_Closed_Mins,
                                                     data_source-- Set in the cursor below
                                                                --, inc_mttcreate
                                                                --, inc_mttn
                                                     ,
                                                     description,
                                                     ...
                                                     summary,
                                                     component_name,
                                                     directv_remedy_number)
                 SELECT   -- atomic data 1 x 1
                          hd.incident_number,
                          hd.alarm_identifier,
                          hd.alarm_type,
                          hd.assigned_group,
                          ...
                          hd.total_time_spent,
                          hd.total_transfers,
                          hd.total_pending_time_in_minutes,
                          hd.urgency-- Atomic Dates
                          ,
                          metric_utilities.convert_to_date (hd.closed_date),
                          metric_utilities.convert_to_date (
                             hd.estimated_resolution_date
                          ),
                          metric_utilities.convert_to_date (
                             hd.initial_notification_date
                          ),
                          metric_utilities.convert_to_date (hd.isolation_date),
                          metric_utilities.convert_to_date (
                             hd.last__assigned_date
                          ),
                          metric_utilities.convert_to_date (
                             hd.last_acknowledged_date
                          ),
                          metric_utilities.convert_to_date (
                             hd.last_modified_date
                          ),
                          metric_utilities.convert_to_date (hd.last_occurrence),
                          metric_utilities.convert_to_date (
                             hd.last_resolved_date
                          ),
                          ...
                          (SELECT   metric_utilities.convert_to_date (h.t2)
                            ...
                            WHERE   h.entryid = hd.incident_number)
                             in_progress_date-- this is the original Cisco Remedy code left in for readability
                          ...
                          TRUNC (
                             ...
                             AS week_opened-- Calculated Flags
                          ...
                          CASE
                             WHEN hd.status = 5
                                  AND hd.resolution_category_tier_3 like
                                        '%Duplicate%'
                             THEN
                                'Y'
                             ELSE
                                'N'
                          END
                             AS closed_as_duplicate_flag,
                          CASE
                             WHEN hd.status = 5
                                  AND hd.resolution_category =
                                        'ROS Infrastructure'
                             THEN
                                'Y'
                             ELSE
                                'N'
                          END
                             AS closed_as_ros_infrastr_flag,
                          CASE
                             WHEN hd.status = 5 AND hd.status_reason = 1500
                             THEN
                                'Y'
                             ELSE
                                'N'
                          END
                             AS closed_as_merged_flag,
                          CASE
                             WHEN hd.resolution_category_tier_2 =
                                     'Scheduled Maint'
                             THEN
                                'Y'
                             ELSE
                                'N'
                          END
                             AS sched_maint_flag-- Calculated Counts
                          ,
                          NVL (
                             (  SELECT   COUNT (hw.work_log_type)
                                 ...
                                 WHERE   hw.work_log_type IN (4201)
                                         AND hd.incident_number =
                                               hw.incident_number
                              GROUP BY   hw.incident_number),
                             0
                          )
                             AS num_cust_escalations,
                          NVL (
                             (  SELECT   COUNT (hw.work_log_type)
                                            num_customer_updates
                                  FROM   aradmin.hpd_worklog@DTVREMP1.CISCO.COM hw
                                 WHERE   hw.work_log_type IN
                                               (5105,
                                                6000,
                                                7000,
                                                8000,
                          ...
                          * (24 * 60)
                             alarm_to_notify_mins--(SELECT MIN (time_to_notify)/60
                                                 --        FROM rdr_ticket rdr
                                                 --       WHERE rdr.incident_number = hd.incident_number
                                                 --       GROUP BY rdr.incident_number
                                                 -- ) AS
                          ...
                          * (24 * 60)
                             alarm_to_isolate_mins-- (SELECT MIN (time_to_isolate)/60
                                                  --        FROM rdr_ticket rdr
                                                  --       WHERE rdr.incident_number = hd.incident_number
                                                  --       GROUP BY rdr.incident_number
                                                  -- ) AS
                          ...
                          CASE
                             ...
                             THEN
                                 ...
                                 * (24 * 60))
                                - NVL (total_pending_time_in_minutes, 0)
                             ...
                             ELSE
                                NULL   
                          END
                             alarm_to_resolve_mins-- (SELECT MIN (time_to_resolve)/60
                                                  --    FROM rdr_ticket rdr
                                                  --   WHERE rdr.incident_number = hd.incident_number
                                                  --   GROUP BY rdr.incident_number
                                                  -- ) AS
                          ...
                          * (24 * 60)
                             alarm_to_close_mins--(SELECT MIN (time_to_close)/60
                                                --    FROM rdr_ticket rdr
                                                --   WHERE rdr.incident_number = hd.incident_number
                                                --   GROUP BY rdr.incident_number
                                                --) AS
                          ...
                          * (24 * 60)
                             alarm_to_create_mins-- (SELECT MIN (time_to_create)/60
                                                 --        FROM rdr_ticket rdr
                                                 --       WHERE rdr.incident_number = hd.incident_number
                                                 --       GROUP BY rdr.incident_number
                                                 -- ) AS
                          ...
                          (metric_utilities.convert_to_date (hd.Closed_Date)
                           - metric_utilities.convert_to_date (hd.Submit_Date))
                          * (24)
                             AS Tkt_Create_To_Closed_Hrs,
                          (metric_utilities.convert_to_date (hd.Isolation_Date)
                           - metric_utilities.convert_to_date (hd.Submit_Date))
                          * (24 * 60)
                             AS Tkt_Create_To_Isolate_Mins,
                          (metric_utilities.convert_to_date (
                              hd.Initial_Notification_Date
                           )
                           - metric_utilities.convert_to_date (hd.Submit_Date))
                          * (24 * 60)
                             AS Tkt_Create_To_Notify_Mins-- tkt_create_to_resolved
                          ,
                          CASE
                             WHEN     hd.last_resolved_date IS NOT NULL
                             ...
                             THEN
                                 ...
                                 * (24))
                                - NVL (total_pending_time_in_minutes, 0) / 60
                             WHEN     hd.last_resolved_date IS NULL
                                  AND hd.closed_date IS NOT NULL
                                  AND hd.submit_date IS NOT NULL
                             ...
                             ELSE
                                NULL   
                          END
                             AS tkt_create_to_resolved_hrs,
                          (metric_utilities.convert_to_date (
                              hd.service_restored_date
                           )
                           - metric_utilities.convert_to_date (hd.Submit_Date))
                          * 1440
                             AS Tkt_Create_To_Serv_Restor_Mins-- tkt_isolate_to_resolved
                          ,
                          CASE
                             WHEN     hd.last_resolved_date IS NOT NULL
                             ...
                             ELSE
                                NULL   
                          END
                             AS tkt_isolate_to_resolved_mins-- tkt_closed_to_resolved
                          ,
                          CASE
                             WHEN     hd.last_resolved_date IS NOT NULL
                                  AND hd.closed_date IS NOT NULL
                             ...
                             ELSE
                                NULL  
                          END
                             AS tkt_resolved_to_closed_mins,
                          'DTVREMP1.CISCO.COM'-- Set inc_mttcreate in cursor below
                                              --, CASE
                          ...
                          ,
                          REPLACE (
                             REPLACE (hd.description, CHR (10), CHR (32)),
                             CHR (9),
                             CHR (32)
                          ),
                          DECODE (hd.submitter, 'WEBSERVICES', 'Y', 'N')
                             AS system_created,
                          v_last_updated-- REPORTED_NETWORK_CONDITION
                                        --   ENUMID   REMEDY VALUE              PRIORITY
                                        --   0        Network Event             3
                                        --   1        Degraded Service          2
                                        --   2        Outage                    1
                          ,
                          hd.reported_network_condition,
                          DECODE (hd.notification_required,
                                  0,
                                  'Y',
                                  1,
                                  'N')
                             AS notification_required,
                          hd.follow_up_required,
                          hdr.resolution,
                          hdr.description,
                          hdr.component_name,
                          hdr.directv_remedy_number
                   FROM   aradmin.hpd_help_desk@DTVREMP1.CISCO.COM hd,
                          aradmin.hpd_help_desk_report@DTVREMP1.CISCO.COM hdr
                  WHERE       hd.company NOT IN ('ROS-LAB', 'Cisco ROS')
                          AND hd.incident_number = hdr.incident_number
                          AND hd.incident_number = tkt_rec.incident_number --in (select incident_number from im_stage_data_ids_to_pull_dtv)
         ...
         EXCEPTION
            WHEN OTHERS
            THEN
               INSERT INTO im_dtv_failed_records (incident_number)
                 VALUES   (tkt_rec.incident_number);

               v_msg := SQLERRM;

               metric_utilities.log_writer (
                  'DTV',
                  'IM_GET_DATA_DTV ERROR ' || tkt_rec.incident_number,
                  SYSDATE,
                  v_msg
               );
         END;
      END LOOP;
        ...
        SELECT   i.incident_number,
                 NVL (
                    ...
                    AS cnt
          BULK   COLLECT
          INTO   my_ticket_list_an, my_task_count_list
          FROM   im_stage_data_dtv i, aradmin.tms_task@DTVREMP1.CISCO.COM t
      ...
      -- all others
      -- -----------------------------------------------------------------------------
      SELECT   -- 0. ticket_list
               im.incident_number-- 1. company_id
               ,
               im.company,
               auto_notify_flag-- 2. auto_notify_flag
               ...
               ,
               im.in_progress_date,
               im.reported_date-- 4. inc_mttc
                               -- 6. met_mttc sla
               ,
               im.alarm_to_create_mins-- 5. inc_mttn
                                      -- 7. mttn_sla_met
               ,
               im.tkt_create_to_notify_mins,
               im.submitter-- 8-11. met_Px_SLA
               ,
               im.priority,
        ...
        BULK   COLLECT
        INTO   -- 0. ticket_list
               my_ticket_list-- 1. company_id
               ,
               my_company_list-- 2. auto_notify_flag
               ,
               my_auto_notify_list-- 3. alarm_to_analyze
               ,
               my_in_progress_date_list,
               my_reported_date_list-- 4. inc_mttc
                                    -- 6. met_mttc sla
               ,
               my_alarm_to_crt_mins_list-- 5. inc_mttn
                                        -- 7. mttn_sla_met
               ,
               my_tkt_create_to_not_mins_list,
               my_submitter_list-- 8-11. met_Px_SLA
               ,
               my_priority_list,
               my_reptd_network_cond_list,
               my_tkt_create_to_res_hrs_list
        FROM   im_stage_data_dtv im
       WHERE   1 = 1 AND submit_date > TO_DATE ('11/12/2008', 'dd/mm/yyyy');
      ...
      LOOP
         ...
         v_alarm_to_analyze_mins :=
              (my_in_progress_date_list (i) - my_reported_date_list (i))
            * 24
            * 60;

         IF v_alarm_to_analyze_mins < 0
         THEN
            v_alarm_to_analyze_mins := NULL;
         END IF;
         ...
         IF my_alarm_to_crt_mins_list (i) <= 7 AND v_inc_mttcreate = 'Y'
         THEN
         ...
         -- 7. met_mttn_sla
         IF my_tkt_create_to_not_mins_list (i) <= 15 AND v_inc_mttn = 'Y'
         THEN
            v_mttn_sla_met := 'Y';
         ELSE
            v_mttn_sla_met := 'N';
         ...
         IF     NOT (reported_net_cond_is_blank)
            ...
            AND my_tkt_create_to_res_hrs_list (i) <= 4
         THEN
            v_mttr_p1_sla_met := 'Y';
         ELSE
            IF NOT (reported_net_cond_is_blank)
         ...
         THEN
            v_mttr_p1_sla_met := 'Y';
         ELSE
            IF reported_net_cond_is_blank AND my_priority_list (i) = 1
            THEN
               v_mttr_p1_sla_met := 'N';
            ELSE
               v_mttr_p1_sla_met := NULL;
         ...
         THEN
            v_mttr_p2_sla_met := 'Y';
         ELSE
            IF reported_net_cond_is_blank AND my_priority_list (i) = 2
            THEN
               v_mttr_p2_sla_met := 'N';
            ELSE
               v_mttr_p2_sla_met := NULL;
         ...
         IF     NOT (reported_net_cond_is_blank)
            ...
            AND my_tkt_create_to_res_hrs_list (i) <= 24
         THEN
            v_mttr_p3_sla_met := 'Y';
         ELSE
            IF NOT (reported_net_cond_is_blank)
         ...
         THEN
            v_mttr_p3_sla_met := 'Y';
         ELSE
            IF reported_net_cond_is_blank AND my_priority_list (i) = 3
            THEN
               v_mttr_p3_sla_met := 'N';
            ELSE
               v_mttr_p3_sla_met := NULL;
         ...
         BEGIN
            UPDATE   im_stage_data_dtv
               SET   -- 1. company_id
                     companyid = v_company_id-- 2. auto_notify_flag
                     ,
                     auto_notify_flag = v_auto_notify_flag-- 3. alarm_to_analyze
                     ,
                     alarm_to_analyze_mins = v_alarm_to_analyze_mins-- 4. inc_mttc
                     ,
                     inc_mttcreate = v_inc_mttcreate-- 6. met_mttc_sla
                     ,
                     mttcreate_sla_met = v_mttcreate_sla_met-- 5. inc_mttc
                     ,
                     inc_mttn = v_inc_mttn-- 7. met_mttn_sla
                     ,
                     mttn_sla_met = v_mttn_sla_met-- 8-11. met_Px_sla
                     ,
         ...
         EXCEPTION
            ...
            THEN
               ...
               metric_utilities.log_writer (
                  'ALL',
                  'IM_GET_DATA ERROR - update stage_dtv',
                  SYSDATE,
                  v_msg
               );
         END;
   ...
   EXCEPTION
      ...
      THEN
         ...
         o_im_status := 'FAILURE';
         metric_utilities.log_writer ('ALL',
                                      'IM_GET_DATA_DTV ERROR',
                                      SYSDATE,
                                      v_msg);
   END;


   PROCEDURE im_update_history_dtv (o_im_status OUT VARCHAR)
   IS
      v_msg   VARCHAR2 (1000);
   BEGIN
      ...
      INSERT INTO im_history_data_dtv (incident_number,
                                       alarm_identifier,
                                       alarm_type,
                                       assigned_group,
                                       assigned_support_company,
   ...
   BEGIN
      ...
      im_kpi_ru_proc_dtv (i_start_date,
                          i_stop_date,
                          i_period_id,
                          i_period_type,
                          i_period_name,
      ...
      IF i_period_type = 'DAY'
      THEN
         -- this is an external procedure
         im_backlog_buckets_proc_dtv (i_start_date,
                                      i_stop_date,
                                      i_period_id,
                                      i_period_type,
                                      i_period_name,
                                      o_im_status);
         -- this is an external procedure
         im_mttr_buckets_proc_dtv (i_start_date,
                                   i_stop_date,
                                   i_period_id,
                                   i_period_type,
                                   i_period_name,
   ...
   BEGIN
      ...
      METRIC_UTILITIES.TRUNC_TABLE ('pm_ids_to_pull_dtv');
      METRIC_UTILITIES.TRUNC_TABLE ('pm_stage_data_dtv');
      COMMIT;

      -- ---------------------------------------------------------------
      --    5. get pbi ids to pull this time
      INSERT INTO pm_ids_to_pull_dtv (pbi_id)
         SELECT   ppi.problem_investigation_id
           FROM   aradmin.pbm_problem_investigation@DTVREMP1.CISCO.COM ppi
          WHERE   metric_utilities.convert_to_date (last_modified_date) >
                     i_start_date
         UNION
         ...
         SELECT   psda.request_id01
          ...
          WHERE       psd.solution_database_id = psda.request_id02
                  ...
                  AND metric_utilities.convert_to_date (
                        psd.last_modified_date
                     ) > i_start_date;
      ...
      INSERT INTO pm_stage_data_dtv (-- atomic fields (1-1 mapping)
                                 pbi_id,
                                 submit_date,
                                 last_modified_date,
                                 status,
           ...
           SELECT   -- atomic fields (1-1 mapping)
                    DISTINCT
                    (ppi.problem_investigation_id),
                    metric_utilities.convert_to_date (ppi.submit_date)
                       AS submit_date,
                    metric_utilities.convert_to_date (ppi.last_modified_date)
                    ...
                    'DTV',
                    ppi.investigation_category,
                    ppi.generic_categorization_tier_1 
             FROM   aradmin.pbm_problem_investigation@DTVREMP1.CISCO.COM ppi,
                    ...
                    (SELECT   ppi.problem_investigation_id pbi_id,
                              metric_utilities.convert_to_date (h.t8)
                                 closed_date,
                              NVL (
                                 metric_utilities.time_diff (
                                    metric_utilities.convert_to_date (
                    ...
                    (SELECT   ppi.problem_investigation_id AS pbi_id,
                              metric_utilities.convert_to_date (h.t6)
                                 resolved_date,
                              NVL (
                                 metric_utilities.time_diff (
                                    metric_utilities.convert_to_date (
                    ...
                    -- count_touches
                    (  SELECT   ppi.problem_investigation_id AS pbi_id,
                                COUNT (piw.problem_investigation_id)
                                   AS num_touches
                         FROM   aradmin.pbm_problem_investigation@DTVREMP1.CISCO.COM ppi,
                                aradmin.pbm_investigation_worklog@DTVREMP1.CISCO.COM piw
                        WHERE   ppi.problem_investigation_id =
                                   piw.problem_investigation_id
                    ...
                    (  SELECT   ppi.problem_investigation_id pbi_id,
                        ...
                        WHERE   ppi.problem_investigation_id =
                                   piw.problem_investigation_id
                                AND SUBSTR (piw.description, 1, 17) =
                                      'Formal escalation'
                    ...
                    (  SELECT   pia.request_id02,
                                NVL (
                                   COUNT(CASE
                                            WHEN SUBSTR (pia.request_id01, 1, 3) =
                                                    'CRQ'
                                            THEN
                                               1
                                            ELSE
                                ...
                                NVL (
                                   ...
                                   AS num_SDB_associations,
                                NVL (
                                   COUNT(CASE
                                            WHEN SUBSTR (pia.request_id01, 1, 3) NOT IN
                                                       ('CRQ', 'INC', 'SDB')
                                            THEN
                                               1
                                            ELSE
            ...
            WHERE       1 = 1
                    ...
                    AND ppi.problem_investigation_id =
                          count_escalations.pbi_id(+)
                    AND ppi.problem_investigation_id =
                          count_associations.request_id02(+)
                    AND ppi.problem_investigation_id IN
                             (  SELECT   DISTINCT pbi_id FROM pm_ids_to_pull_dtv)
      ...
      DECLARE
         ...
         IS
              SELECT   pbi_id
                FROM   pm_stage_data_dtv
            ORDER BY   pbi_id;

         CURSOR pbi_assoc_cur (
            i_pbi_id   IN            VARCHAR2
         )
         IS
            SELECT   form_name01,
                     form_name02,
                     ...
                     association_type01
              FROM   aradmin.pbm_investigation_associations@DTVREMP1.CISCO.COM pia
             WHERE   request_id02 IS NOT NULL -- AND substr(request_id02, length(request_id02)-3) = 341;
         ...
         LOOP
            ...
            LOOP
               ...
               IF pbi_assoc_rec.association_type01 = '31000'
                  AND SUBSTR (pbi_assoc_rec.request_id01, 1, 3) = 'INC'
               THEN
                  UPDATE   pm_stage_data_dtv
                     SET   pbi_created_by_inc_flag = 'Y'
                   WHERE   pm_stage_data_dtv.pbi_id = pbi_assoc_rec.request_id02;
               END IF;

               -- if the pbi was created by an change
               IF pbi_assoc_rec.association_type01 = '31000'
                  AND SUBSTR (pbi_assoc_rec.request_id01, 1, 3) = 'CRQ'
               THEN
                  UPDATE   pm_stage_data_dtv
               ...
               IF pbi_assoc_rec.association_type01 = '30000'
                  AND SUBSTR (pbi_assoc_rec.request_id01, 1, 3) = 'CRQ'
               THEN
                  UPDATE   pm_stage_data_dtv
               ...
               IF pbi_assoc_rec.association_type01 = '30000'
                  AND SUBSTR (pbi_assoc_rec.request_id01, 1, 3) = 'SDB'
               THEN
                  UPDATE   pm_stage_data_dtv
      ...
      METRIC_UTILITIES.TRUNC_TABLE ('pm_sbd_ids_to_pull_dtv');
      METRIC_UTILITIES.TRUNC_TABLE ('pm_sdb_stage_data_dtv');

      -- ---------------------------------------------------------------
      --    18. get sdb ids to pull this time
      INSERT INTO pm_sbd_ids_to_pull_dtv (sdb_id)
         SELECT   sdb.solution_database_id
           FROM   aradmin.pbm_solution_database@DTVREMP1.CISCO.COM sdb
          WHERE   metric_utilities.convert_to_date (sdb.last_modified_date) >
                     i_start_date;

      -- ---------------------------------------------------------------
      --    19. insert into sdb_stage_data
      INSERT INTO pm_sdb_stage_data_dtv (sdb_id,
                                     submit_date,
                                     company,
                                     site,
                                     assigned_group,
         ...
         SELECT   solution_database_id sdb_id,
                  metric_utilities.convert_to_date (sdb.submit_date)
                     AS submit_date,
                  sdb.company,
                  sdb.site,
                  sdb.assigned_group,
          ...
          WHERE   sdb.solution_database_id IN
                        (SELECT   DISTINCT sdb_id FROM pm_sbd_ids_to_pull_dtv);
   ...
   EXCEPTION
      ...
      THEN
         ...
         o_pm_status := 'FAILURE';
         metric_utilities.log_writer ('ALL',
                                      'PM_GET_DATA_DTV ERROR',
                                      SYSDATE,
                                      v_msg);
   END pm_get_data_dtv;

   PROCEDURE pm_update_history_dtv (o_pm_status OUT VARCHAR)
   IS
      v_msg   VARCHAR2 (1000);
   BEGIN
      DELETE FROM   pm_history_data
            WHERE   data_source = 'DTV'
                    AND pbi_id IN (     SELECT   pbi_id FROM pm_stage_data_dtv);

      COMMIT;

      -- ---------------------------------------------------------------
      --    9. insert into pm_history
      INSERT INTO pm_history_data (pbi_id,
                                   submit_date,
                                   last_modified_date,
                                   status,
                                   priority,
      ...
      DELETE FROM   pm_sdb_history_data
            WHERE   data_source = 'DTV'
                    AND sdb_id IN
                             (     SELECT   sdb_id FROM pm_sdb_stage_data_dtv);

      -- ---------------------------------------------------------------
      --    21. insert into sdb_history
      INSERT INTO pm_sdb_history_data (sdb_id,
                                       submit_date,
                                       company,
                                       site,
                                       assigned_group,
   ...
   PROCEDURE pm_rollup_data_dtv (o_pm_status OUT VARCHAR)
   IS
      v_msg      VARCHAR2 (1000);
      v_status   VARCHAR2 (15);
   BEGIN
--      metric_utilities.refresh_view ('pm_rollup_company_site_VW',
--                                     'A',
--                                     v_status);
--      metric_utilities.refresh_view ('pm_rollup_team_assignee_VW',
--                                     'A',
--                                     v_status);
--      metric_utilities.refresh_view ('pm_rollup_tkt_metrics_VW',
--                                     'AF',
--                                     v_status);
--      metric_utilities.refresh_view ('pm_rollup_tkt_data_VW', 'A', v_status);
--      metric_utilities.refresh_view ('pm_sdb_rollup_comp_site_VW',
--                                     'A',
--                                     v_status);
--      metric_utilities.refresh_view ('pm_sdb_rollup_team_assn_VW',
--                                     'A',
--                                     v_status);
--      metric_utilities.refresh_view ('pm_rollup_sdb_tkt_data_VW',
--                                     'A',
--                                     v_status);
      o_pm_status := 'SUCCESS';
   EXCEPTION
   ...
   IS
      v_msg   VARCHAR2 (1000);
   BEGIN
      metric_utilities.log_writer ('CM',
                                   'cm_dtv_get_stage_data procedure start',
                                   SYSDATE,
                                   NULL);
      ...
      INSERT INTO cm_change_id_list (request_id)
         (SELECT   cic.infrastructure_change_id
            FROM   aradmin.chg_infrastructure_change@DTVREMP1.CISCO.COM cic
           WHERE   metric_utilities.convert_to_date (cic.last_modified_date) BETWEEN i_start_date
                                                                                 AND  i_stop_date
          UNION
          SELECT   ca.request_id02
            FROM   aradmin.chg_associations@DTVREMP1.CISCO.COM ca
           WHERE   metric_utilities.convert_to_date (ca.modified_date) BETWEEN i_start_date
                                                                           AND  i_stop_date
          UNION
          SELECT   cw.infrastructure_change_id
            FROM   aradmin.chg_worklog@DTVREMP1.CISCO.COM cw
           WHERE   metric_utilities.convert_to_date (cw.last_modified_date) BETWEEN i_start_date
                                                                                AND  i_stop_date);
      ...
      -- Fill CM_STAGE_DATA
      INSERT INTO CM_STAGE_DATA (-- ATOMIC FIELDS
                                 ...
                                 SUBMIT_DATE-- Calculated Dates
                                 ,
                                 COMPLETED_DATE_H,
                                 CLOSED_DATE_H,
                                 MONTH_COMPLETED_H,
                                 MONTH_OPENED-- Calculated Flags
                                 ,
                                 CRQ_CREATED_BY_INC,
                                 ...
                                 CHG_IS_EXPEDITED_FLAG-- Calculated Counts
                                 ,
                                 NUM_INC_FROM_THIS_CRQ-- Calculated Times
                                 ,
                                 TIME_TO_COMPLETE_DAYS-- Data Source
                                 ,
                                 DATA_SOURCE)
         SELECT   -- Atomic Fields
                  ...
                  cic.urgency-- Calculated Values
                  ,
                  metric_utilities.enum_values (1200,
                                                1000000568,
                                                cic.change_timing)
                     AS change_timing_value,
                  REPLACE (
                     REPLACE (cic.customer_internet_e_mail, CHR (10), ' '),
                     CHR (13),
                     ' '
                  )
                     AS customer_email,
                  REPLACE (REPLACE (cic.description, CHR (10), ' '),
                           CHR (13),
                           ' ')
                     AS description,
                  metric_utilities.enum_values (1200,
                  ...
                  metric_utilities.enum_values (1200,
                                                1000000150,
                                                cic.status_reason)
                     AS status_reason-- Atomic Dates
                  ,
                  metric_utilities.convert_to_date (
                     cic.acknowledgment_end_date
                  )
                     AS acknowledgment_end_date,
                  metric_utilities.convert_to_date (cic.actual_end_date)
                     AS actual_end_date,
                  metric_utilities.convert_to_date (cic.actual_start_date)
                     AS actual_start_date,
                  metric_utilities.convert_to_date (cic.completed_date)
                     AS completed_date,
                  metric_utilities.convert_to_date (cic.earliest_start_date)
                     AS earliest_start_date,
                  metric_utilities.convert_to_date (cic.last_modified_date)
                     AS last_modified_date,
                  metric_utilities.convert_to_date (cic.next_target_date)
                     AS next_target_date,
                  metric_utilities.convert_to_date (cic.requested_end_date)
                     AS requested_end_date,
                  metric_utilities.convert_to_date (cic.requested_start_date)
                     AS requested_start_date,
                  metric_utilities.convert_to_date (cic.resolution_end_date)
                     AS resolution_end_date,
                  metric_utilities.convert_to_date (cic.response_target_date)
                     AS response_target_date,
                  metric_utilities.convert_to_date (cic.rfc_date) AS rfc_date,
                  metric_utilities.convert_to_date (cic.scheduled_end_date)
                     AS scheduled_end_date,
                  metric_utilities.convert_to_date (cic.scheduled_start_date)
                     AS scheduled_start_date,
                  metric_utilities.convert_to_date (cic.submit_date)
                     AS submit_date-- Calculated Dates
                  ,
                  ...
                  TO_CHAR ( (SELECT   metric_utilities.convert_to_date (t10)
                               FROM   aradmin.H1200@DTVREMP1.CISCO.COM h
                              WHERE   h.entryid = cic.request_id), 'MONTH')
                     AS month_completed_h,
                  TO_CHAR (
                     metric_utilities.convert_to_date (cic.submit_date),
                     'MONTH'
                  )
                     AS month_opened-- Calculated Flags
                  ,
                  ca_set_flag.flag AS crq_created_by_INC,
                  CASE WHEN cic.change_type = 2000 THEN 'Y' ELSE 'N' END
                     AS chg_type_is_change_flag,
                  CASE
                  ...
                  CASE
                     ...
                     ELSE
                        'N'
                  END
                     AS chg_cancelled_rescheduled_flag,
                  CASE WHEN cic.change_timing = 1000 THEN 'Y' ELSE 'N' END
                     AS chg_is_emergency_flag,
                  CASE
                  ...
                  CASE
                     ...
                     ELSE
                        'N'
                  END
                     AS chg_is_successful_flag,
                  CASE WHEN cic.change_timing = 2000 THEN 'Y' ELSE 'N' END
                     AS chg_is_expedited_flag-- Calculated Counts
                  ,
                  NVL (count_incidents.num_inc_from_this_CRQ, 0)
                     AS num_inc_from_this_CRQ-- Calculated Times
                  ,
                  (CASE
                      ...
                      ELSE
                        NULL   
                   END)
                     AS time_to_complete_days,
                  'DTV'
           FROM   aradmin.chg_infrastructure_change@DTVREMP1.CISCO.COM cic-- count_incidents created by this change
                  ,
                  (  SELECT   cic.infrastructure_change_id
                                 AS infrastructure_change_id,
                   ...
                   GROUP BY   cic.infrastructure_change_id) count_incidents,
                  (  SELECT   cic.infrastructure_change_id
                                 AS infrastructure_change_id,
                       ...
                       FROM   aradmin.chg_infrastructure_change@DTVREMP1.CISCO.COM cic,
                              (SELECT   ca.request_id02,
                                 ...
                                 FROM   aradmin.chg_associations@DTVREMP1.CISCO.COM ca)
                              ca_flag
                      WHERE   cic.infrastructure_change_id =
                                 ca_flag.request_id02
                   GROUP BY   cic.infrastructure_change_id) ca_set_flag
          WHERE   cic.infrastructure_change_id =
                     ca_set_flag.infrastructure_change_id(+)
                  AND cic.infrastructure_change_id =
                        count_incidents.infrastructure_change_id(+)
   ...
   PROCEDURE cm_update_history_dtv (o_cm_status OUT VARCHAR)
   IS
      v_msg   VARCHAR2 (1000);
   BEGIN
      metric_utilities.log_writer (
         'CM',
         'cm_dtv_update_history_data procedure start',
         SYSDATE,
         NULL
      );
      ...
      INSERT INTO CM_HISTORY_DATA (-- ATOMIC FIELDS
                                   INFRASTRUCTURE_CHANGE_ID,
                                   ASCHG,
                                   ASGRP,
                                   ASSIGNED_TO,
      ...
      metric_utilities.log_writer (
         'CM',
         'cm_dtv_update_history_data procedure end',
         SYSDATE,
         NULL
      );
      o_cm_status := 'SUCCESS';
   EXCEPTION
      ...
      THEN
         ...
         metric_utilities.log_writer (
            'CM',
            'cm_dtv_get_stage_data procedure ERROR',
            SYSDATE,
            v_msg
         );
   ...
   PROCEDURE cm_rollup_data_dtv (o_cm_status OUT VARCHAR)
   IS
      v_msg      VARCHAR2 (1000);
      v_status   VARCHAR2 (15);
   BEGIN
      metric_utilities.refresh_snapshot ('CM_ROLLUP_OPENCOUNTSVW', v_status);
      metric_utilities.refresh_snapshot ('CM_ROLLUP_CLOSEDCOUNTSVW',
                                         v_status);
      metric_utilities.refresh_snapshot ('CM_ROLLUP_OPENCOUNTSBYEMPVW',
                                         v_status);
      metric_utilities.refresh_snapshot ('CM_ROLLUP_CLOSEDCOUNTSBYEMPVW',
                                         v_status);
      o_cm_status := 'SUCCESS';
   EXCEPTION
      WHEN OTHERS
