...
-- Create Table as Select
        drop table mybooks_etl_staging;
        create table mybooks_etl_staging
        (
           isbn       varchar2(20)
...
-- Example of ETL
        -- LOAD
        insert into mybooks_etl_staging
        (
           isbn
          ,category
          ,title
        ...
        select
          ...
          ,to_char(retail - cost,'$99.99') as profit   -- transforming to new value with formatting
        from books.books;
...
select * from hats;
        ...
        select * from mybooks_etl_staging;
        -- -----------------------------------------------------------------------------
...
-- Searching forward
select substr('APPLES ARE FOR BOW AND ARROW TARGETS',4,5) from dual;
-- Searching backwards
...
commit;
          -- -----------------------------------------------------------------------------
          -- ETL BUSINESS LOGIC FROM TUITION DATA
          -- A.   Source Field:              tuition.groupid
...
select to_date('2016-JAN-01', 'YYYY-MON-DD') + 5 from dual;
select to_date('20160121', 'YYYYMMDD') + 5 from dual;
select to_date('21012016', 'DDMMYYYY') + 5 from dual;
...
select null+5, nvl(null,0) from dual;

-- -------------------------------------------------------
--  NVL2(s1,r1,r2)	  If s1 NULL, return r2 else r1
--  Substitute a value for null, or another value if it is not null
select * from cruises.scores;
insert into cruises.scores values (4,null,88);
insert into cruises.scores values (5,null,66);
insert into cruises.scores values (6,90,null);
select test_score, updated_test_score, 
        NVL2(updated_test_score, updated_test_score, test_score) as final 
from cruises.scores;
...
select nullif(23.000, 23) as testnullif from dual;
          ...
          select tcsname, instname,  nvl2(instname,instname,tcsname) as i_name
          from tuition;
--        If s1 NULL, return r2 else r1
-- -----------------------------------------------------------------------------
-- 7. CHAPTER 6 DECODE
-- -------------------------------------------------------
