...
select racer_id, endtime, starttime, endtime - starttime from davidtest;
    ...
    SELECT 
        to_char(starttime, 'dd-MON-yy HH:MI:SS')
      ,  to_char(endtime, 'dd-MON-yy HH:MI:SS')
      , to_char(trunc(((endtime - starttime) * 24) / 3600), 'FM9900') || ': Hours'
      , to_char((endtime-starttime) * 1440), 'FM00'))) || ': Minutes'
    FROM runners;   
       TO_CHAR(TRUNC(MOD(((endtime-starttime) * 1440, 3600) / 60), 'FM00'))) || ':' ||
       TO_CHAR(MOD(((endtime-starttime) * 1440, 60), 'FM00')) AS MIN_TO_HOUR FROM runners;
    select floor(endtime-starttime) || ' DAYS ' ||
    mod(floor((endtime-starttime)*24),24) || ' HOURS ' ||
    mod(floor((endtime-starttime)*24*60),60)|| ' MINUTES ' ||
    mod(floor((endtime-starttime)*24*60*60),60) || ' SECONDS' race_time  from runners;
...
-- How would you turn the number of days into the number of hours? How many hours in a day?
SELECT 
   TO_CHAR(TRUNC((600.5 * 60) / 3600), 'FM9900') || ':' ||
   TO_CHAR(TRUNC(MOD((600.5 * 60), 3600) / 60), 'FM00') || ':' ||
   TO_CHAR(MOD((600.5 * 60), 60), 'FM00') AS MIN_TO_HOUR FROM DUAL;
